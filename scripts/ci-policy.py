...
def classify(paths: list[str]) -> dict[str, bool]:
    def documentation(path: str) -> bool:
        ...
        return (
            path in DOC_FILES
            or (path.endswith(".md") and ("/" not in path or path.startswith(DOC_MARKDOWN_DIRS)))
            or path.startswith("docs/assets/")
        )

    return {
        "code": not paths or any(not documentation(path) for path in paths),
        "markdown": any(path.endswith(".md") or path == ".markdownlint.json" for path in paths),
    }
...
def main() -> None:
    ...
    if args.gate:
        check_gate(json.loads(os.environ["JOB_RESULTS"]), os.environ["CODE_JOBS"].split())
        return
